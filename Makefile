...
pkgdatadir = $(datadir)/zest
pkgincludedir = $(includedir)/zest
pkglibdir = $(libdir)/zest
pkglibexecdir = $(libexecdir)/zest
...
CONFIG_HEADER = $(top_builddir)/src/config/zest-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/zest-config.h.in \
...
ACLOCAL = ${SHELL} /home/sorin/ZestCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/sorin/ZestCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/sorin/ZestCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/sorin/ZestCoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = zest-cli
BITCOIN_DAEMON_NAME = zestd
BITCOIN_GUI_NAME = zest-qt
BITCOIN_TX_NAME = zest-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/sorin/ZestCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = zest
PACKAGE_BUGREPORT = www.zestcoin.io
PACKAGE_NAME = Zest Core
PACKAGE_STRING = Zest Core 1.0.0
PACKAGE_TARNAME = zest
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/sorin/ZestCoin
abs_srcdir = /home/sorin/ZestCoin
abs_top_builddir = /home/sorin/ZestCoin
abs_top_srcdir = /home/sorin/ZestCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/sorin/ZestCoin/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_zest_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_zest_coverage.info test_zest.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Zest-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/zest-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/zest-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/zest-config.h
$(top_srcdir)/src/config/zest-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/zest-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Zest-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Zest-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Zest-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_zest.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_zest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_zest_filtered.info: test_zest.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_zest_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_zest_coverage.info: baseline_filtered_combined.info test_zest_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zest_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_zest_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zest_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_zest.coverage/.dirstamp:  test_zest_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_zest.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_zest.coverage/ total.coverage/ $(OSX_APP)
